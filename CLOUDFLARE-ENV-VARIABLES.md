...
**重要**：Production 环境不支持 txt 格式的变量！

- ✅ **Preview 环境**：可以配置 txt 格式的变量
- ❌ **Production 环境**：不能配置 txt 格式的变量

**解决方案**：
- 对于敏感信息（API 密钥等），使用 **Secret** 类型
- 对于非敏感信息（URL 等），使用 **Text** 类型
- 不要使用 txt 文件格式
...
- **使用 Secret 类型**存储敏感信息（API 密钥、密码等）
...
- 确保变量名拼写正确
- 检查变量是否在正确的环境中配置
- 确保 Production 环境使用 Secret 或 Text 类型，而不是 txt 格式
